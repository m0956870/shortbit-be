...
const signupUser = async (req, res, next) => {
    ...
    try {
        ...
        const existingUser = await User.findOne(findVal).lean();
        if (existingUser) throw new ApiError("User already exist!", 400);

        const otp = "1234";
        const otp_expiry = new Date(Date.now() + 2 * 60 * 1000);

        await User.create({ name, email, password, phone_number, otp, otp_expiry, user_name: Date.now(), user_id: Date.now() });
